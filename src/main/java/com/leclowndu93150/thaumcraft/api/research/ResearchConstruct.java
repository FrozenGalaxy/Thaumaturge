...
public record ResearchConstruct(int xSize, int ySize, int zSize, List<String> cells, AspectList cost) {
    public static final Codec<ResearchConstruct> CODEC = RecordCodecBuilder.<ResearchConstruct>create(instance -> instance.group(
            ...
            AspectList.CODEC.optionalFieldOf("cost", AspectList.EMPTY).forGetter(ResearchConstruct::cost)
    ).apply(instance, ResearchConstruct::new)).validate(construct ->
            construct.cells().size() == construct.xSize() * construct.ySize() * construct.zSize()
