...
public final class GolemHelper {
    ...
    private static Bindings impl() {
        if (bindings == null) {
            throw new IllegalStateException("GolemHelper used before Thaumcraft bound it");
        }
        return bindings;
    }
    ...
    public static AABB getBoundsForArea(ISealEntity seal) {
        ...
        return new AABB(
                ...
                .expandTowards(
                        sealPos.face().getStepX() != 0 ? (seal.getArea().getX() - 1) * sealPos.face().getStepX() : 0.0,
