...
public final class WandHandRenderer {
    private static final float BASE_SCALE_FACTOR = 0.8F;
    private static final float ITEM_SCALE = 0.4F;
    private static final float EQUIP_DIP = 0.6F;
    ...
    public static void onRenderHand(RenderHandEvent event) {
        ...
        float swingSin = Mth.sin(swing * Mth.PI);
        float swingSqrtSin = Mth.sin(Mth.sqrt(swing) * Mth.PI);

        poseStack.pushPose();
        poseStack.translate(mirror * -swingSqrtSin * 0.4F,
                Mth.sin(Mth.sqrt(swing) * Mth.PI * 2.0F) * 0.2F,
                -swingSin * 0.2F);
        poseStack.translate(mirror * 0.7F * BASE_SCALE_FACTOR,
                -0.65F - equip * EQUIP_DIP,
                -0.9F * BASE_SCALE_FACTOR);
        poseStack.mulPose(Axis.YP.rotationDegrees(mirror * 45.0F));
        poseStack.mulPose(Axis.YP.rotationDegrees(mirror * -Mth.sin(swing * swing * Mth.PI) * 20.0F));
        poseStack.mulPose(Axis.ZP.rotationDegrees(mirror * -swingSqrtSin * 20.0F));
        poseStack.mulPose(Axis.XP.rotationDegrees(-swingSqrtSin * 80.0F));
