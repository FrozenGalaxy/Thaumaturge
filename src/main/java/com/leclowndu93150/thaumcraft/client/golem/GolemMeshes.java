...
public final class GolemMeshes {
    private static final Map<ResourceLocation, TCMesh> CACHE = new ConcurrentHashMap<>();
    private static final float DEFAULT_NORMAL_Y = 1.0F;

    private GolemMeshes() {}

    public static TCMesh get(ResourceLocation meshLocation) {
        return CACHE.computeIfAbsent(meshLocation, location -> {
            try {
                ...
                Thaumcraft.LOGGER.error("Failed to load mesh {}", location, e);
                return TCMesh.EMPTY;
            }
        });
    }
