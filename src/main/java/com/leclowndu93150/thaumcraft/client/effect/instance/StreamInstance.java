...
public abstract class StreamInstance implements IFXInstance {
    ...
    public record Snapshot(double[][] points, float[][] colours, double[] radii,
                           double originX, double originY, double originZ, float texSlice, float start) {}
    ...
    public final void tick() {
        ...
        if (this.radiusBase > MIN_RADIUS) {
            this.trail.add(new TrailPoint(radius,
                    this.posX - this.originX, this.posY - this.originY, this.posZ - this.originZ));
        } else {
