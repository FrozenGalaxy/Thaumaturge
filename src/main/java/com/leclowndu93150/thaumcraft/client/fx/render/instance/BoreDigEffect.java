    ...
    public void tick() {
        ...
        for (int index = 0; index < this.particlesPerTick; index++) {
            ...
            if (random.nextInt(SPARKLE_CHANCE) == 0) {
                BoreSparkleData options = new BoreSparkleData(
                        this.boreEntityId, destination.x, destination.y, destination.z,
                        SPARKLE_RED, SPARKLE_GREEN_BASE + random.nextFloat() * SPARKLE_GREEN_RANGE, SPARKLE_BLUE);
                this.level.addParticle(options, x, y, z, 0.0, 0.0, 0.0);
            } else {
                BoreParticlesData options = new BoreParticlesData(
                        this.state, this.boreEntityId, destination.x, destination.y, destination.z,
                        0.0, 0.0, 0.0);
                this.level.addParticle(options, x, y, z, 0.0, 0.0, 0.0);
            }
        }
