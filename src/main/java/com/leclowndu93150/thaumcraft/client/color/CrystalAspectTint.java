...
public record CrystalAspectTint(int fallback) implements ItemTintSource {
    public static final MapCodec<CrystalAspectTint> MAP_CODEC = RecordCodecBuilder.mapCodec(
            instance -> instance.group(
                    ExtraCodecs.RGB_COLOR_CODEC.optionalFieldOf("fallback", 0xFFFFFF)
                            .forGetter(CrystalAspectTint::fallback)
            ).apply(instance, CrystalAspectTint::new)
    );
