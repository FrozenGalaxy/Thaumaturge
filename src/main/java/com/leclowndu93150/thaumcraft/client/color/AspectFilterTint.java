...
public record AspectFilterTint(int defaultColor) implements ItemTintSource {
    public static final MapCodec<AspectFilterTint> MAP_CODEC = RecordCodecBuilder.mapCodec(
            instance -> instance.group(
                    ExtraCodecs.RGB_COLOR_CODEC.fieldOf("default").forGetter(AspectFilterTint::defaultColor)
            ).apply(instance, AspectFilterTint::new)
    );
    ...
    public int calculate(ItemStack stack, @Nullable ClientLevel level, @Nullable LivingEntity entity) {
        ...
        if (aspect != null && level != null) {
            return level.registryAccess().lookupOrThrow(IAspect.REGISTRY_KEY)
                    ...
                    .orElse(0xFF000000 | defaultColor);
        }
