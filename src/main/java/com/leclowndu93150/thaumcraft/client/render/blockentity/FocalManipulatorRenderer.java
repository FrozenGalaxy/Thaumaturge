...
public final class FocalManipulatorRenderer implements BlockEntityRenderer<BlockEntityFocalManipulator, FocalManipulatorRenderState> {
    ...
    @Override
    public void extractRenderState(BlockEntityFocalManipulator table, FocalManipulatorRenderState state, float partialTicks,
        ...
        for (AspectInstance instance : entries) {
            ItemStackRenderState itemState = new ItemStackRenderState();
            itemModelResolver.updateForTopItem(itemState, EssentiaCrystalFactory.of(instance.aspect()),
                    ItemDisplayContext.GROUND, table.getLevel(), null, 0);
    ...
    private static void submitGlow(SubmitNodeCollector collector, PoseStack poseStack, float ticks,
    ...
    private void submitRay(PoseStack poseStack, SubmitNodeCollector collector, float angle, int num,
