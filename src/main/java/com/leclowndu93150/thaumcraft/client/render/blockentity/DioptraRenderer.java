...
public final class DioptraRenderer implements BlockEntityRenderer<BlockEntityDioptra, DioptraRenderState> {
    ...
    private static final RenderType SIDE_TYPE = RenderType.create("tc_dioptra_side",
    ...
    @Override
    public void extractRenderState(BlockEntityDioptra dioptra, DioptraRenderState state, float partialTicks,
    ...
    private static void sideQuad(VertexConsumer buffer, PoseStack.Pose pose,
                                 float x0, float z0, float x1, float z1, int color) {
        float yBottom = 1.0F;
        float yTop = 2.0F;
    ...
    public AABB getRenderBoundingBox(BlockEntityDioptra blockEntity) {
        ...
        return new AABB(pos.getX() - 0.3, pos.getY() - 0.3, pos.getZ() - 0.3,
                pos.getX() + 1.3, pos.getY() + 2.3, pos.getZ() + 1.3);
    }
}
