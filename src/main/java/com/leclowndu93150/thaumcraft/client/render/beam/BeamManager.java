package com.leclowndu93150.thaumcraft.client.render.beam;

import com.leclowndu93150.thaumcraft.TCIds;
import com.leclowndu93150.thaumcraft.client.render.stream.PolyCone;
...
public final class BeamManager {
    ...
    public static void onTick(LevelTickEvent.Post event) {
        ...
        Iterator<ArcInstance> ait = ARCS.iterator();
        while (ait.hasNext()) {
            ArcInstance a = ait.next();
            a.tick();
            if (a.isExpired()) ait.remove();
        }
        Iterator<BoltInstance> bit = BOLTS.iterator();
        while (bit.hasNext()) {
            BoltInstance b = bit.next();
            b.tick();
            if (b.isExpired()) bit.remove();
        }
        Iterator<BeamInstance> bmit = BEAMS.iterator();
        while (bmit.hasNext()) {
            BeamInstance b = bmit.next();
            b.tick();
            if (b.isExpired()) bmit.remove();
        }
    }

    @SubscribeEvent
    public static void onRender(RenderLevelStageEvent.AfterTranslucentParticles event) {
        if (ARCS.isEmpty() && BOLTS.isEmpty()) return;
    ...
    private static void renderBeam(PoseStack poseStack, MultiBufferSource bufSource, BeamInstance beam, Vec3 camPos, float partialTick) {
        ...
        poseStack.mulPose(new org.joml.Quaternionf().rotateX((float)Math.toRadians(90.0)));
        poseStack.mulPose(new org.joml.Quaternionf().rotateZ((float)Math.toRadians(-(180.0F + yaw))));
        poseStack.mulPose(new org.joml.Quaternionf().rotateX((float)Math.toRadians(pitch)));
        poseStack.mulPose(new org.joml.Quaternionf().rotateY((float)Math.toRadians(rot)));
        ...
        for (int t = 0; t < 3; t++) {
            poseStack.mulPose(new org.joml.Quaternionf().rotateY((float)Math.toRadians(60.0)));
