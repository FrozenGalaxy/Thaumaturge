...
public final class CardTruth extends TheorycraftCard {
    @Override
    ...
    public Component name() {
        return Component.translatable("card.thaumcraft.truth.name");
    }

    @Override
    public Component description() {
        return Component.translatable("card.thaumcraft.truth.text");
    }

    @Override
    public boolean activate(ServerPlayer player, IResearchTableData data) {
        data.addCategoryTotal(TCResearchCategories.ELDRITCH, Mth.nextInt(player.getRandom(), 10, 25));
        data.addBonusDraws(1);
        WarpHelper.addWarp(player, 3, WarpType.TEMPORARY);
        return true;
    }
}
