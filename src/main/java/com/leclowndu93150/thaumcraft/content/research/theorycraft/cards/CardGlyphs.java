...
public final class CardGlyphs extends TheorycraftCard {
    @Override
    ...
    public Component name() {
        return Component.translatable("card.thaumcraft.glyph.name");
    }

    @Override
    public Component description() {
        return Component.translatable("card.thaumcraft.glyph.text");
    }

    @Override
    public boolean activate(ServerPlayer player, IResearchTableData data) {
        data.addCategoryTotal(CardHelper.randomCategory(player), Mth.nextInt(player.getRandom(), 10, 20));
        data.addCategoryTotal(TCResearchCategories.ELDRITCH, Mth.nextInt(player.getRandom(), 10, 20));
