...
public final class ObsidianTotemFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        if (!groundState.is(Blocks.GRASS_BLOCK) && !groundState.is(Blocks.SAND) && !groundState.is(Blocks.DIRT)
                && !groundState.is(Blocks.STONE) && !groundState.is(Blocks.NETHERRACK)) {
            return false;
        }
        int clearance = 1;
