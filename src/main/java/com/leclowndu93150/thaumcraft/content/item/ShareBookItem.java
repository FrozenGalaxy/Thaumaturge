...
public class ShareBookItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        if (level.isClientSide()) return InteractionResultHolder.sidedSuccess(player.getItemInHand(hand), level.isClientSide());
        ...
        if (player.isCrouching() && !stack.has(TCDataComponents.DISCOVERED_ASPECTS)){
            ...
            if (profile == null || profile.equals(player.getGameProfile())) return InteractionResultHolder.pass(player.getItemInHand(hand));
            ...
            if (aspects == null || knowledge == null || !(currentKnowledge instanceof PlayerKnowledge playerKnowledge)) return InteractionResultHolder.pass(player.getItemInHand(hand));
