...
public final class BaubleEvents {
    ...
    public static void onDeath(LivingDeathEvent event) {
        ...
        ItemStack charm = ThaumcraftCuriosCompat.extractCurio(player, TCItems.CHARM_UNDYING.get());
        if (charm.isEmpty()) {
            return;
        }
    ...
    public static void onPickupXp(PlayerXpEvent.PickupXp event) {
        ...
        if (!(player instanceof ServerPlayer serverPlayer)
                ...
                || !ModList.get().isLoaded(TCIds.CURIOS)
                || !ThaumcraftCuriosCompat.isCurioEquipped(player, TCItems.CURIOSITY_BAND.get())) {
            return;
        }
