...
import com.leclowndu93150.thaumcraft.content.taint.block.ITaintBlock;
...
public final class TaintHelper {
    ...
    public static double spreadArea() {
        return ThaumcraftCommonConfig.TAINT_SPREAD_AREA.get();
    }
    ...
    public static void spreadFibres(ServerLevel level, BlockPos pos, boolean force) {
        if (!force && ThaumcraftCommonConfig.WUSS_MODE.get()) {
            return;
        }
        RandomSource random = level.getRandom();
        float saturation = AuraHelper.getFluxSaturation(level, pos);
        float mod = 0.001F + saturation * 2.0F;
        double rate = ThaumcraftCommonConfig.TAINT_SPREAD_RATE.get() / 100.0;
