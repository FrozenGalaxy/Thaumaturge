    ...
    static void processDecorations(GenContext ctx) {
        for (BlockPos pos : ctx.decoUrn) {
            if (ctx.level.isEmptyBlock(pos.above())) {
                ...
                BlockState urn = roll < 0.025F ? TCBlocks.LOOT_URN_RARE.get().defaultBlockState()
                        : roll < 0.1F ? TCBlocks.LOOT_URN_UNCOMMON.get().defaultBlockState()
            ...
            if (exposed == 1 && !isAdjacentToEldritchDeco(ctx, pos)) {
                for (Direction dir : Direction.values()) {
                    ...
                    if (ctx.level.getBlockState(front).isAir()) {
                        ctx.level.setBlock(front, TCBlocks.ELDRITCH_CRAB_SPAWNER.get().defaultBlockState()
                                .setValue(BlockEldritchCrabSpawner.FACING, dir), 3);
                        break;
                    }
                }
            }
