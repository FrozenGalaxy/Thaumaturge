    ...
    public void handleEntityEvent(byte id) {
        if (id == 3) {
            for (int a = 0; a < SPLOSION_COUNT; a++) {
                this.level().addParticle(TCParticles.TAINT_SPLOSION.get(),
                        this.getX(), this.getY() + this.random.nextFloat() * this.getBbHeight(), this.getZ(),
            ...
            this.level().playLocalSound(this.getX(), this.getY(), this.getZ(),
                    ...
                    1.0F, this.random.nextFloat() * 0.1F + 0.9F, false);
        } else {
