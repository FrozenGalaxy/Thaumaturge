...
@EventBusSubscriber
public final class EssentiaSourceFXTracker {
    ...
    private EssentiaSourceFXTracker() {}
    ...
    public static void onLevelTick(LevelTickEvent.Post event) {
        ...
        while (it.hasNext()) {
            ...
            if (now - p.tickStamp >= FLUSH_TICKS) {
                TCParticleDispatch.broadcastEssentiaStream(server, p.from, p.to, p.color, p.typeTag, p.count, p.scale, p.extend, p.my);
                it.remove();
            }
        }
    }
