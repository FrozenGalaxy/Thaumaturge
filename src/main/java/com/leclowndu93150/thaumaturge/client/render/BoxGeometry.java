...
public final class BoxGeometry {
    ...
    public static void box(PoseStack.Pose pose, VertexConsumer buffer,
                           float x0, float y0, float z0, float x1, float y1, float z1,
                           int u, int v, int dx, int dy, int dz,
                           int texW, int texH, int tint, int light, boolean mirror) {
        ...
        float[] v7 = {xb, y0, z1};
        float[] v8 = {xa, y0, z1};
        quad(pose, buffer, tint, light, texW, texH, mirror, v6, v2, v3, v7,
                u + dz + dx, v + dz, u + dz + dx + dz, v + dz + dy);
        quad(pose, buffer, tint, light, texW, texH, mirror, v1, v5, v8, v4,
                u, v + dz, u + dz, v + dz + dy);
        quad(pose, buffer, tint, light, texW, texH, mirror, v6, v5, v1, v2,
                u + dz, v, u + dz + dx, v + dz);
        quad(pose, buffer, tint, light, texW, texH, mirror, v3, v4, v8, v7,
                u + dz + dx, v + dz, u + dz + dx + dx, v);
        quad(pose, buffer, tint, light, texW, texH, mirror, v2, v1, v4, v3,
                u + dz, v + dz, u + dz + dx, v + dz + dy);
        quad(pose, buffer, tint, light, texW, texH, mirror, v5, v6, v7, v8,
    ...
    private static void quad(PoseStack.Pose pose, VertexConsumer buffer, int tint, int light,
                             int texW, int texH, boolean mirror,
        ...
        for (int i = 0; i < 4; i++) {
            buffer.addVertex(pose, verts[i][0], verts[i][1], verts[i][2]).setColor(tint)
