...
public final class StageNavigatorWidget {
    ...
    public static void render(
            GuiGraphics graphics, Font font, int x, int y, int width, int currentStage, int totalStages) {
        String label = (currentStage + 1) + " / " + totalStages;
        int labelWidth = font.width(label);
        graphics.drawString(font, Component.literal(label), x + width / 2 - labelWidth / 2, y + 2, TEXT_COLOR, false);
    }
}
