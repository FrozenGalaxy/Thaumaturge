...
public final class TaintacleModel extends HierarchicalModel<AbstractTaintacle> {
    ...
    public TaintacleModel(ModelPart root, int length) {
        ...
        applyScale(parent.getChild("orb"));
        applyScale(parent.getChild("end"));
    }

    private static void applyScale(ModelPart part) {
        part.xScale = SEGMENT_SCALE;
        part.yScale = SEGMENT_SCALE;
        part.zScale = SEGMENT_SCALE;
    }
    ...
    public void setupAnim(
        ...
        for (int k = 0; k < length - 1; k++) {
            segments[k].xRot = 0.15F * fi * Mth.sin(ageInTicks * 0.1F * fs - k / 2.0F);
            segments[k].zRot = 0.1F / fi * Mth.sin(ageInTicks * 0.15F - k / 2.0F);
        }
    }
}
