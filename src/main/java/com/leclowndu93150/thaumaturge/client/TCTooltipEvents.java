...
public final class TCTooltipEvents {
    ...
    public static void onItemTooltip(ItemTooltipEvent event) {
        ...
        if (runic > 0) {
            event.getToolTip().add(1, Component.translatable("tooltip.thaumaturge.runic_charge", runic)
                    .withStyle(ChatFormatting.GOLD));
        }
        int warp = WarpHelper.getFinalWarp(event.getItemStack(), event.getEntity());
        if (warp > 0) {
            event.getToolTip().add(1, Component.translatable("item.thaumaturge.warping")
                    .withStyle(ChatFormatting.DARK_PURPLE));
        }
        if (event.getItemStack().getItem() instanceof IRechargable rechargable) {
            event.getToolTip().add(1, Component.translatable("tooltip.thaumaturge.charge",
        ...
        if (event.getItemStack().getItem() instanceof BlockItem blockItem && isStabiliser(blockItem.getBlock())
            ...
            event.getToolTip().add(Component.translatable("tooltip.thaumaturge.infusion_stabiliser")
                    .withStyle(ChatFormatting.DARK_PURPLE));
        }
    }
