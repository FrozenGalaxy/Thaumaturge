...
public class MenuTurretBasic extends AbstractContainerMenu {
    ...
    public MenuTurretBasic(int containerId, Inventory playerInventory, RegistryFriendlyByteBuf buf) {
        this(TCMenus.TURRET_BASIC.get(), containerId, playerInventory,
                playerInventory.player.level().getEntity(buf.readVarInt()) instanceof EntityTurretCrossbow t ? t : null,
                AMMO_X, AMMO_Y);
    }

    protected MenuTurretBasic(MenuType<?> type, int containerId, Inventory playerInventory,
                              @Nullable EntityTurretCrossbow turret, int slotX, int slotY) {
    ...
    public static void open(Player player, EntityTurretCrossbow turret) {
        if (player instanceof ServerPlayer serverPlayer) {
            serverPlayer.openMenu(new SimpleMenuProvider(
                    (id, inv, p) -> new MenuTurretBasic(TCMenus.TURRET_BASIC.get(), id, inv, turret, AMMO_X, AMMO_Y),
                    turret.getDisplayName()), buf -> buf.writeVarInt(turret.getId()));
        }
    }
