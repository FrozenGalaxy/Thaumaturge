...
public final class BlockWorkQueues {
    ...
    public record BreakerTask(BlockPos pos, BlockState source, UUID playerId, boolean fx, boolean silk, int fortune,
            float strength, float durability, float durabilityMax, int delay, float visCost) {

        BreakerTask withDelay(int delay) {
            return new BreakerTask(pos, source, playerId, fx, silk, fortune,
                    strength, durability, durabilityMax, delay, visCost);
        }
        ...
        public static final class Builder {
            ...
            public void queue(ServerLevel level) {
                level.getData(TCAttachments.BLOCK_WORK_QUEUES).breakers()
                        .add(new BreakerTask(pos, source, playerId, fx, silk, fortune,
                                strength, durability, durabilityMax, delay, visCost));
            }
        }
    }

    public record SwapperTask(BlockPos pos, @Nullable BlockState source, @Nullable BlockState target,
            boolean consumeTarget, int lifespan, UUID playerId, boolean fx, boolean fancy, int color,
        ...
        SwapperTask spreadTo(BlockPos neighbour) {
            return new SwapperTask(neighbour, source, target, consumeTarget, lifespan - 1, playerId,
        ...
        public static final class Builder {
            ...
            public void queue(ServerLevel level) {
                level.getData(TCAttachments.BLOCK_WORK_QUEUES).swappers()
                        .add(new SwapperTask(pos, source, target, consumeTarget, lifespan, playerId,
                                fx, fancy, color, pickup, silk, fortune, allowSwap, visCost));
            }
        }
    }
