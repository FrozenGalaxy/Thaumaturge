...
public class ItemAlumentum extends Item implements ProjectileItem {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (level instanceof ServerLevel serverLevel) {
            ThrownAlumentum projectile = new ThrownAlumentum(serverLevel, player, itemStack);
            projectile.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, 0.4F, 2.0F);
