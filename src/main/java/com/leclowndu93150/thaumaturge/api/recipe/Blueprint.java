...
public record Blueprint(
        int xSize, int ySize, int zSize, Map<Character, BlueprintPart> keys, List<List<String>> pattern) {
    public static final ResourceKey<Registry<Blueprint>> REGISTRY_KEY =
            ResourceKey.createRegistryKey(ResourceLocation.fromNamespaceAndPath("thaumaturge", "blueprint"));
    ...
    private static final Codec<Map<Character, BlueprintPart>> KEYS_CODEC =
            Codec.unboundedMap(SYMBOL_CODEC, BlueprintPart.CODEC);
    ...
    public static final Codec<Blueprint> CODEC = RecordCodecBuilder.create(i -> i.group(
                    ...
                    KEYS_CODEC.optionalFieldOf("keys", new HashMap<>()).forGetter(Blueprint::keys),
                    Codec.STRING
                            .listOf()
                            .listOf()
                            .optionalFieldOf("pattern", new ArrayList<>())
                            .forGetter(Blueprint::pattern))
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, Blueprint> STREAM_CODEC = StreamCodec.composite(
            ...
            ByteBufCodecs.VAR_INT,
            Blueprint::zSize,
            ByteBufCodecs.map(HashMap::new, ByteBufCodecs.fromCodec(SYMBOL_CODEC), BlueprintPart.STREAM_CODEC),
            Blueprint::keys,
    ...
    public Blueprint {
        if (xSize <= 0 || ySize <= 0 || zSize <= 0) {
            throw new IllegalArgumentException("Blueprint dimensions can't be zero or negative : xSize: " + xSize
                    + ", ySize: " + ySize + ", zSize: " + zSize);
        }
        if (pattern.size() != ySize) {
            throw new IllegalArgumentException(
                    "Not enough layers for blueprint : Expected: " + ySize + ", Found: " + pattern.size());
        }
        Set<Character> definedKeys = new HashSet<>();
        for (List<String> layer : pattern) {
            if (layer.size() != xSize) {
                throw new IllegalArgumentException(
                        "Not enough rows for blueprint : Expected: " + xSize + ", Found: " + layer.size());
            }
            for (String row : layer) {
                if (row.length() != zSize) {
                    throw new IllegalArgumentException(
                            "Not enough columns for blueprint : Expected: " + zSize + ", Found: " + row.length());
                }
        ...
        if (!keys.keySet().containsAll(definedKeys)) {
            ...
            throw new IllegalArgumentException(
                    "Not all characters in the pattern have a corresponding key defined. Missing keys: " + missingKeys);
        }
    ...
    private @Nullable BlueprintPart partForChar(char c) {
        if (c == ' ') return null;
        if (!keys.containsKey(c)) throw new IllegalArgumentException("No blueprint part defined for character: " + c);
        return keys.get(c);
    }
}
