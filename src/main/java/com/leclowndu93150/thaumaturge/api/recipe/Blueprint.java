...
public record Blueprint(int xSize, int ySize, int zSize, Map<Character, BlueprintPart> keys, List<List<String>> pattern) {
    ...
    private static final Codec<Map<Character, BlueprintPart>> KEYS_CODEC = Codec.unboundedMap(
            SYMBOL_CODEC,
            BlueprintPart.CODEC
    );
    ...
    public static final Codec<Blueprint> CODEC = RecordCodecBuilder.create(i -> i.group(
            ...
            Codec.STRING.listOf().listOf().optionalFieldOf("pattern", new ArrayList<>()).forGetter(Blueprint::pattern)
